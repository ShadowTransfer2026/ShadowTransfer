#!/bin/bash
# FILENAME: dinov3_mcl.sh
#
# SLURM job script for DINOv3 + mCL-LC training.
# Supports Gilbreth and Anvil clusters via comment-swap blocks.
#
# ---- SBATCH common ----
#SBATCH --nodes=1
#SBATCH --gpus-per-node=1
#SBATCH --cpus-per-task=1
#SBATCH --exclude=g005
#SBATCH --mem=64G
#SBATCH --time=6:59:59

# ---- SBATCH cluster-specific (uncomment ONE block) ----
# --- Gilbreth ---
##SBATCH -A sukkusur
##SBATCH --partition=a100-40gb
##SBATCH --gres=gpu:1
##SBATCH --qos=standby

# --- Anvil ---
#SBATCH -A cis260282-gpu
#SBATCH -p gpu

# ---- Server paths (uncomment ONE block) ----
# --- Gilbreth ---
# cd /home/mittal53/ShadeMaps/python/dinov3
# PYTHON_BIN=/scratch/gilbreth/mittal53/ShadeMaps/conda_envs/satmae_cuda12/bin/python

# --- Anvil ---
cd /anvil/projects/x-cis260282/ShadeMaps/python/dinov3
PYTHON_BIN=/anvil/projects/x-cis260282/satmae_cuda12/bin/python

# ---- Module / conda (uncomment ONE block) ----
# --- Gilbreth ---
# module load conda
# module load cuda/12.1.1
# module load cudnn/9.2.0.82-12
# conda activate /scratch/gilbreth/mittal53/ShadeMaps/conda_envs/satmae_cuda12

# --- Anvil ---
module purge
module load modtree/gpu
module load cuda/12.6.1
module load anaconda
conda activate /anvil/projects/x-cis260282/satmae_cuda12

# Distributed training env vars (single node)
export MASTER_ADDR=localhost
export MASTER_PORT=12355
export WORLD_SIZE=1
export RANK=0
export LOCAL_RANK=0
export LOCAL_WORLD_SIZE=1
export PYTHONUNBUFFERED=1

# ---- Run training ----
if [ "$MODE" == "loco" ]; then
    $PYTHON_BIN -u train_dinov3_mcl.py \
        --mode loco \
        --base_data_root ${BASE_DATA_ROOT} \
        --resolution ${RESOLUTION} \
        --fold_id ${FOLD_ID} \
        --model_name dinov3_vits16 \
        --weights_path ${WEIGHT_DIR} \
        --batch_size 8 \
        --epochs 100 \
        --lr 0.0001 \
        --img_size 384 \
        --output_dir ${OUTPUT_DIR} \
        --num_workers 1 \
        --eval_boundary_tolerant \
        --early_stopping_patience 15 \
        --comparison_inference_dir ${COMPARISON_INFERENCE_DIR} \
        --comparison_data_root ${COMPARISON_DATA_ROOT} \
	--use_mcl \
        --use_bane \
        --lambda_fl 0.001 \
        --lambda_sl 0.001 \
        --lambda_lc 0.0005 \

elif [ "$MODE" == "all" ]; then
    $PYTHON_BIN -u train_dinov3_mcl.py \
        --mode all \
        --base_data_root ${BASE_DATA_ROOT} \
        --resolution ${RESOLUTION} \
        --model_name dinov3_vits16 \
        --weights_path ${WEIGHT_DIR} \
        --batch_size 8 \
        --epochs 100 \
        --lr 0.0001 \
        --img_size 384 \
        --output_dir ${OUTPUT_DIR} \
        --num_workers 1 \
        --eval_boundary_tolerant \
        --early_stopping_patience 15 \
        --comparison_inference_dir ${COMPARISON_INFERENCE_DIR} \
        --comparison_data_root ${COMPARISON_DATA_ROOT} \
	--use_mcl \
        --use_bane \
        --lambda_fl 0.001 \
        --lambda_sl 0.001 \
        --lambda_lc 0.0005 \

elif [ "$MODE" == "single" ]; then
    $PYTHON_BIN -u train_dinov3_mcl.py \
        --mode single \
        --data_root ${DATA_ROOT} \
        --model_name dinov3_vits16 \
        --weights_path ${WEIGHT_DIR} \
        --batch_size 8 \
        --epochs 100 \
        --lr 0.0001 \
        --img_size 384 \
        --output_dir ${OUTPUT_DIR} \
        --num_workers 1 \
        --eval_boundary_tolerant \
        --early_stopping_patience 15 \
        --comparison_inference_dir ${COMPARISON_INFERENCE_DIR} \
        --comparison_data_root ${COMPARISON_DATA_ROOT} \
	--use_mcl \
        --use_bane \
        --lambda_fl 0.001 \
        --lambda_sl 0.001 \
        --lambda_lc 0.0005 \

else
    echo "ERROR: Unknown MODE=${MODE}"
    exit 1
fi