"""
Dataset loader for shadow detection with FDA and geocoordinate support.
Expects standard PyTorch segmentation format.
Supports SegDesic domain adaptation through geographic coordinates.
"""
...
class ShadowDataset(Dataset):
    """
    Shadow Detection Dataset with FDA and geocoordinate support
    ...
    def __init__(self, root_dir, split='train', img_size=384, augment=False, 
        ...
        if self.use_fda and split == 'train':
            ...
            try:
                ...
                print(f"Warning: Could not import FDA transform: {e}")
                self.use_fda = False
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            Dictionary with:
                ...
                - filename: str
                - lat: Tensor (float32) if geo_metadata provided
                - lon: Tensor (float32) if geo_metadata provided
        """
        ...
        if self.geo_metadata is not None:
            if img_name in self.geo_metadata:
                ...
                # Only print warning once per unique filename (avoid spam)
                if not hasattr(self, '_warned_files'):
                    self._warned_files = set()
                if img_name not in self._warned_files:
                    print(f"Warning: No geocoordinates found for {img_name}, using default")
                    self._warned_files.add(img_name)
...
class UnlabeledDataset(ShadowDataset):
    ...
    Returns images and coordinates only (no masks).
    Used for SegDesic domain adaptation in LOCO mode.
    """
    ...
    def __getitem__(self, idx):
        """Returns image and coordinates only (no mask)"""
        # Get pre-computed paths
        img_name = self.img_files[idx]
        img_path = self.img_paths[idx]
...
class Dataset(ShadowDataset):
    """
    Shadow Detection Dataset wrapper
    ...
    def __init__(self, root_dir, split='train', img_size=384, augment=False, 
        ...
        print(f"Data {split} dataset initialized:")
        print(f"  Images: {len(self.img_files)}")
        print(f"  Image size: {img_size}x{img_size}")
        print(f"  Augmentation: {augment}")
        if use_fda:
            print(f"  FDA enabled with L={fda_L}")
        if geo_metadata_path:
            print(f"  Geocoordinates enabled")
...
class ShadowDatasetMCL(ShadowDataset):
    ...
    def __init__(self, root_dir, split='train', img_size=384, augment=False,
                 ...
                 use_fda=False, fda_target_root=None, fda_L=0.01,
                 geo_metadata_path=None,
                 use_pseudo_cloud=True, cloud_p=0.3, shadow_p=0.2):
        """
        Args:
            ...existing args...
            use_pseudo_cloud: Whether to apply pseudo-cloud augmentation
            cloud_p: Probability of cloud augmentation
            shadow_p: Probability of cloud shadow augmentation
        """
        super().__init__(
            root_dir, split, img_size, augment,
            samples_per_dir, random_seed, selected_filenames,
        ...
        self.use_pseudo_cloud = use_pseudo_cloud
        
        # Pseudo-cloud augmentation
        if self.use_pseudo_cloud and split == 'train':
            from utils.pseudo_cloud_aug import CombinedAerialAugmentation
            self.pseudo_cloud_aug = CombinedAerialAugmentation(
                cloud_p=cloud_p,
                shadow_p=shadow_p
            )
        else:
            self.pseudo_cloud_aug = None
...
def get_dataloaders(data_root=None, base_data_root=None, mode='single', 
                   ...
                   use_fda=False, fda_target_root=None, fda_L=0.01,
                   geo_metadata_path=None,
                   use_mcl=False, use_pseudo_cloud=True):
    """
    Create train, validation, and test dataloaders.
    For LOCO mode with geo_metadata, also creates unlabeled target domain loader for UDA.
    ...
    Args:
        ...
        fda_target_root: Path to target domain images for FDA
        fda_L: Low-frequency ratio for FDA (beta parameter)
        geo_metadata_path: Path to geocoordinate metadata JSON (for SegDesic)
        
    Returns:
        Dictionary of dataloaders
        - For standard mode: {'train', 'val', 'test'}
        - For LOCO + SegDesic mode: {'train', 'val', 'test', 'target'}
    """
    if use_mcl:
    ...
    if mode == 'single':
        ...
        # For SegDesic: use target city as unlabeled target domain
        if geo_metadata_path is not None:
    ...
    if use_mcl:
        train_kwargs['use_pseudo_cloud'] = use_pseudo_cloud
    ...
    # Create unlabeled target domain dataset for UDA (only in LOCO mode with SegDesic)
    target_dataset = None
...
if __name__ == "__main__":
    ...
    # Test with geocoordinates
    if 'lat' in sample:
        print(f"Latitude: {sample['lat']}")
        print(f"Longitude: {sample['lon']}")