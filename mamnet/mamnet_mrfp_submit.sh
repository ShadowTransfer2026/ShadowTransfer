...
do
    ...
    do
        ...
        sbatch --output=${outputfile} \
               --job-name=${name} \
               --export=MODE=loco,BASE_DATA_ROOT=${BASE_DATA_ROOT},RESOLUTION=${res},FOLD_ID=${fold_id},OUTPUT_DIR=${OUTPUT_DIR},USE_CONTRAST=1,EVAL_TOLERANT=1,BOUNDARY_TOLERANCE=${BOUNDARY_TOLERANCE},EARLY_STOPPING_PATIENCE=10,USE_MRFP_PLUS=${USE_MRFP_PLUS},HRFP_PROB=${HRFP_PROB},NP_PROB=${NP_PROB},HRFP_PLUS_PROB=${HRFP_PLUS_PROB},HRFP_BN_STD=${HRFP_BN_STD},COMPARISON_INFERENCE_DIR=${COMPARISON_INFERENCE_DIR},COMPARISON_DATA_ROOT=${COMPARISON_DATA_ROOT} \
               mamnet_mrfp.sh
    done
done
