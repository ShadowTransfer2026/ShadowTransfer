"""
Dataset loader for shadow detection.
...
class ShadowDataset(Dataset):
    ...
    def __init__(self, root_dir, split='train', img_size=384, augment=False, 
             ...
             geo_metadata_path=None):  # NEW PARAMETER
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            Dictionary with:
                ...
                Main image and mask are BOTH augmented together for segmentation.
        """
        # Get paths
        img_name = self.img_files[idx]
        img_path = self.img_paths[idx]
...
class Dataset(ShadowDataset):
    ...
    def __init__(self, root_dir, split='train', img_size=384, augment=False, samples_per_dir=None, random_seed=42,
             use_fda=False, fda_target_root=None, fda_L=0.01, geo_metadata_path=None):
        super().__init__(root_dir, split, img_size, augment, samples_per_dir, random_seed,
                    selected_filenames=None, use_fda=use_fda, fda_target_root=fda_target_root, fda_L=fda_L,
...
def get_dataloaders(data_root=None, base_data_root=None, mode='single', 
    ...
    Args:
        ...
        use_mcl: Use MCL dataset (returns dual augmented views)
        use_pseudo_cloud: Apply pseudo-cloud augmentation (for mCL-LC)
        Create train, validation, and test dataloaders.
