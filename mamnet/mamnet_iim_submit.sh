#!/bin/bash
# FILENAME: mamnet_iim_submit.sh
#
# Queues MAMNet-IIM training jobs on SLURM.
...
# ============================================================
# PART 2: Train LOCO models (MAMNet-IIM)
# ============================================================
echo "Queueing MAMNet-IIM LOCO models..."
...
do
    ...
    do
        ...
        name="mamnet_iim__loco_holdout_${holdout_city}__${res}"
        outputfile="${BASE_PATH}/data/mamnet/${name}.out"
        echo "  - LOCO fold ${fold_id} (holdout: ${holdout_city}) ${res} (IIM)"
        sbatch --output=${outputfile} \
               --job-name=${name} \
               --export=MODE=loco,BASE_DATA_ROOT=${BASE_DATA_ROOT},RESOLUTION=${res},FOLD_ID=${fold_id},OUTPUT_DIR=${OUTPUT_DIR},USE_CONTRAST=1,EVAL_TOLERANT=1,BOUNDARY_TOLERANCE=${BOUNDARY_TOLERANCE},EARLY_STOPPING_PATIENCE=10,IIM_NUM_KERNELS=${IIM_NUM_KERNELS},IIM_KERNEL_SIZE=${IIM_KERNEL_SIZE},II_LOSS_MODE=${II_LOSS_MODE},II_TARGET_RATIO=${II_TARGET_RATIO},II_LOSS_WEIGHT=${II_LOSS_WEIGHT},GAMMA_RANGE_LO=${GAMMA_RANGE_LO},GAMMA_RANGE_HI=${GAMMA_RANGE_HI},COMPARISON_INFERENCE_DIR=${COMPARISON_INFERENCE_DIR},COMPARISON_DATA_ROOT=${COMPARISON_DATA_ROOT} \
               mamnet_iim.sh
    done
done

echo "All MAMNet-IIM jobs queued!"