#!/bin/bash
# FILENAME: mamnet_sib_newmod_submit.sh
#
# Queues MAMNet+SIB diagnostic module ablation jobs on SLURM.
#
...
LOG_DIR="${BASE_PATH}/data/mamnet/sib_newmod_logs"
...
# =====================================================================
# Helper: submit one configuration across all folds and resolutions
#
# Args:
    ...
    for fold_id in 0 1 2; do
        for res in highres; do
            ...
            sbatch \
                --output="${outfile}" \
                --job-name="${name:0:60}" \
                --export=MODE=loco,BASE_DATA_ROOT=${BASE_DATA_ROOT},RESOLUTION=${res},FOLD_ID=${fold_id},OUTPUT_DIR=${OUTPUT_DIR}/${name},FDA_TARGET_ROOT=${fda_tgt},COMPARISON_INFERENCE_DIR=${COMPARISON_INFERENCE_DIR},COMPARISON_DATA_ROOT=${COMPARISON_DATA_ROOT},MAMNET_OUTPUT_DIR=${MAMNET_OUTPUT_DIR},USE_HAAR=${HAAR},USE_VIB=${VIB},USE_CONTENT_AUG=${AUG},ADAPTIVE_BETA=${AB},USE_FDA=${FDA},USE_SAG=${SAG},USE_MULTISCALE_SIB=${MS},USE_CONTRAST=${CTR},FDA_L=${FL},BETA_CONTENT=${BC},BOUNDARY_TOLERANCE=${BT},USE_PASSTHROUGH_GATE=${GATE},USE_MODULE_BYPASS=${MOD_BYPASS},BETA_MAX_MULTIPLIER=${BMM},USE_CACR=${L_CACR},CACR_WEIGHT=${L_CACR_W},CACR_NEG_WEIGHT=${L_CACR_NW},USE_CE_AURC=${L_CE_AURC},CE_AURC_WEIGHT=${L_CE_AURC_W},USE_TENT=${L_TENT},TENT_STEPS=${L_TENT_S},TENT_LR=${L_TENT_LR} \
                mamnet_sib_newmod.sh
        done
    done
}
...
# ─────────────────────────────────────────────────────────────────────────────
echo "========================================================"
echo "  MAMNet + SIB — Diagnostic Module Ablation Submission"
if [ "${DRY_RUN}" -eq 1 ]; then
echo "  MODE: DRY RUN (no jobs submitted)"
fi
echo "========================================================"
echo "  Output:  ${OUTPUT_DIR}"
echo "  Logs:    ${LOG_DIR}/mamnet_sib_N*.out"
echo ""
...
#   §4.4 prediction: TENT should help MAMNet moderately (encoder+decoder)
# =====================================================================
...
echo "  Logs:     tail -f ${LOG_DIR}/mamnet_sib_N*.out"
echo "  Results:  ls ${OUTPUT_DIR}/mamnet_sib_N*/comparison_results.json"
echo ""