...
do
    ...
    do
        ...
        sbatch --output=${outfile} \
               --job-name=${name} \
               --export=MODE=loco,BASE_DATA_ROOT=${BASE_DATA_ROOT},RESOLUTION=${res},FOLD_ID=${fold_id},OUTPUT_DIR=${OUTPUT_DIR},GEO_METADATA_PATH=${GEO_METADATA_PATH},USE_CONTRAST=1,EVAL_TOLERANT=1,BOUNDARY_TOLERANCE=${BOUNDARY_TOLERANCE} \
               mamnet_segdesic.sh
    done
done
