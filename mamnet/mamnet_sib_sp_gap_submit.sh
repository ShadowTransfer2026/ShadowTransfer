...
for fold_id in 0 1 2; do
    ...
    sbatch \
        --output="${outfile}" \
        --job-name="${name}" \
        --export=FOLD_ID=${fold_id},RESOLUTION=${RESOLUTION},VANILLA_CHECKPOINT=${VANILLA_CKPT},C4CLEAN_CHECKPOINT=${C4CLEAN_CKPT},BASE_DATA_ROOT=${BASE_DATA_ROOT},OUTPUT_DIR=${OUTPUT_DIR} \
