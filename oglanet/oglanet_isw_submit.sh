...
do
    ...
    do
        ...
        sbatch --output="${outputfile}" \
               --job-name="${name}" \
               --export=MODE=loco,BASE_DATA_ROOT=${BASE_DATA_ROOT},RESOLUTION=${res},FOLD_ID=${fold_id},OUTPUT_DIR=${OUTPUT_DIR},ISW_MASK_DIR=${mask_dir},ISW_LAMBDA=${ISW_LAMBDA},USE_CONTRAST=1,EVAL_TOLERANT=1,BOUNDARY_TOLERANCE=${BOUNDARY_TOLERANCE},EARLY_STOPPING_PATIENCE=10 \
               oglanet_isw.sh
        echo ""
    done
done
