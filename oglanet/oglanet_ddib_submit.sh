#!/bin/bash
# FILENAME: oglanet_ddib_submit.sh
#
# Queues OGLANet+DDIB training jobs on SLURM.
#
# Run 1 — Full DDIB + SF (C1+C2+C3+SF):            3 jobs (3 folds x 1 res)
# Run 2 — Ablation C3 + SF only:                    3 jobs
# Run 3 — Ablation C1+C2+SF:                        3 jobs
# Run 4 — Ablation C2+C3+SF:                        3 jobs
# Run 5 — No-DDIB baseline:                         3 jobs
# Run 6 — Full DDIB, no skip filters (Option A):    3 jobs
# Run 7 — Skip filters only (no DDIB components):   3 jobs
#
...
submit_loco() {
    ...
    for fold_id in 0 1 2; do
        for res in highres; do
            ...
            name="oglanet_${TAG}__loco_holdout_${holdout}__${res}"
            outfile="${BASE_PATH}/data/oglanet/${name}.out"
...
# =====================================================================
# submit_loco  "ddib_C1C2C3_SF"  1 1 1 1  0.1 0.01 0.001  0

# =====================================================================
# RUN 2: Ablation — C3 + SF only  (Feature Augmentation + Skip Filters)
# =====================================================================
# submit_loco  "ddib_C3_SF"  0 0 1 1  0.1 0.01 0.001  0

# =====================================================================
# RUN 3: Ablation — C1 + C2 + SF  (Disentangle + VIB + Skip Filters)
# =====================================================================
# submit_loco  "ddib_C1C2_SF"  1 1 0 1  0.1 0.01 0.001  0

# =====================================================================
# RUN 4: Ablation — C2 + C3 + SF  (VIB + Augmentation + Skip Filters)
# =====================================================================
submit_loco  "ddib_C2C3_SF"  0 1 1 1  0.1 0.01 0.001  0

# =====================================================================
# RUN 5: No-DDIB baseline  (no DDIB, no skip filters)
# =====================================================================
submit_loco  "ddib_none"  0 0 0 0  0.1 0.01 0.001  0

# =====================================================================
# RUN 6: Option A equivalent — Full DDIB, NO skip filters
# =====================================================================
submit_loco  "ddib_C1C2C3_noSF"  1 1 1 0  0.1 0.01 0.001  0

# =====================================================================
# RUN 7: Skip filters only — no DDIB components
#         (tests skip filtering in isolation)
# =====================================================================
submit_loco  "ddib_SFonly"  0 0 0 1  0.1 0.01 0.001  0
