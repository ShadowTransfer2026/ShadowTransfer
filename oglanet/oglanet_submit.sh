...
do
    ...
    do
        ...
        sbatch --output=${outputfile} \
               --job-name=${name} \
               --export=MODE=single,DATA_ROOT=${data_root},OUTPUT_DIR=${OUTPUT_DIR},USE_CONTRAST=1,EVAL_TOLERANT=1,BOUNDARY_TOLERANCE=2,EARLY_STOPPING_PATIENCE=10 \
               oglanet.sh
    done
done
...
do
    ...
    do
        ...
        sbatch --output=${outputfile} \
               --job-name=${name} \
               --export=MODE=loco,BASE_DATA_ROOT=${BASE_DATA_ROOT},RESOLUTION=${res},FOLD_ID=${fold_id},OUTPUT_DIR=${OUTPUT_DIR},USE_CONTRAST=1,EVAL_TOLERANT=1,BOUNDARY_TOLERANCE=2,EARLY_STOPPING_PATIENCE=10 \
               oglanet.sh
    done
done
