...
class ShadowDatasetEnhancedGSD(ShadowDatasetEnhanced):
    """ShadowDatasetEnhanced with GSD extraction added."""
    def __getitem__(self, idx):
        result = super().__getitem__(idx)
        gsd = get_gsd_from_filename(self.img_files[idx])
        result['gsd'] = torch.tensor(gsd, dtype=torch.float32)
        return result
...
class ShadowDatasetGSDPE(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Apply contrast channel if enabled (before transforms)
        if self.use_contrast:
            image = add_contrast_channel(image)  # Returns 4-channel numpy array
        ...
        if self.augment:
            if self.use_contrast:
                # image is already a numpy array (4-channel), handle without PIL
                mask_arr = np.array(mask)[:, :, None]  # [H, W, 1]
                combined = np.concatenate([np.array(image), mask_arr], axis=2)  # [H, W, 5]
                combined_tensor = transforms.ToTensor()(combined)
                # Apply flips manually
                if torch.rand(1) < 0.5:
                    combined_tensor = transforms.functional.hflip(combined_tensor)
                if torch.rand(1) < 0.5:
                    combined_tensor = transforms.functional.vflip(combined_tensor)
                combined = combined_tensor.numpy().transpose(1, 2, 0)
                image = combined[:, :, :4]   # back to numpy [H, W, 4]
                mask = Image.fromarray(combined[:, :, 4].astype(np.uint8))
            else:
        ...
        # Apply transforms
        if self.use_contrast:
            # Convert 4-channel array: normalize RGB with ImageNet, contrast to [0,1]
            img_array = np.array(image).astype(np.float32)
            rgb = img_array[:, :, :3] / 255.0
            contrast = img_array[:, :, 3:4] / 255.0
            mean = np.array([0.485, 0.456, 0.406])
            std = np.array([0.229, 0.224, 0.225])
            rgb = (rgb - mean) / std
            combined = np.concatenate([rgb, contrast], axis=2)  # [H, W, 4]
            combined = torch.tensor(combined.transpose(2, 0, 1), dtype=torch.float32)
            resize = transforms.Resize((self.img_size, self.img_size))
            image = resize(combined)
        else:
...
def get_dataloaders_gsdpe(data_root=None, base_data_root=None, mode='single', cities=None, resolution=None, batch_size=8, 
                          num_workers=1, img_size=384, use_contrast=False):
    """
    ...
    DatasetClass = ShadowDatasetEnhancedGSD if use_contrast else ShadowDatasetGSDPE

    train_dataset = DatasetClass(
        train_paths, split='train', img_size=img_size, augment=True, use_contrast=use_contrast
    )
    val_dataset = DatasetClass(
        val_paths, split='val', img_size=img_size, augment=False, use_contrast=use_contrast
    )
    test_dataset = DatasetClass(
        test_paths, split='test', img_size=img_size, augment=False, use_contrast=use_contrast
    )
