#!/bin/bash
# FILENAME: oglanet_mrfp.sh
# SLURM job script for OGLANet + MRFP / MRFP+ training.
# Supports single, all, and loco modes.
#
# Server blocks: uncomment the one you need.
# Currently: Gilbreth is active.
...
# ============================================================
echo "========================================"
echo "OGLANet + MRFP Training"
echo "========================================"
echo "Mode:              ${MODE}"
echo "Contrast:          ${CONTRAST_FLAG}"
echo "Tolerant eval:     ${TOLERANT_FLAG}"
echo "Boundary tol:      ${BOUNDARY_TOL_FLAG}"
...
# ============================================================
# Common args shared across all modes
# lr=0.0001  — user-confirmed preference for MRFP-based training
# epochs=100 — full training from scratch (matches base OGLANet)
# optimizer=adamax — matches base OGLANet paper spec
# ============================================================
...
# ============================================================
if [ "$MODE" == "single" ]; then
    $PYTHON_BIN -u train_oglanet_mrfp.py \
        --mode single \
        --data_root ${DATA_ROOT} \
        ${COMMON_ARGS}
