...
model_variant choices (mcl removed; iim, isw, mrfp_plus, fada added):
    ...
    hrda                  — High-Resolution Dense Alignment (MAMNet / DINOv3)
    gsdpe                 — GSD-Positional Encoding (MAMNet / DINOv3)
...
from mamnet.models.mamnet_hrda import create_mamnet_hrda
from mamnet.models.mamnet_gsdpe import MAMNet_GSDPE
...
def get_args():
    ...
    parser.add_argument('--reference_gsd', type=float, default=1.0,
                        help='Reference GSD for GSDPE models')
...
def load_model(args):
    ...
    if args.model_type == 'mamnet':
        ...
        elif args.model_variant == 'hrda':
            model = create_mamnet_hrda(
                num_classes=2, pretrained=False, use_aux=True,
                hr_loss_weight=0.1, use_contrast=True)

        elif args.model_variant == 'gsdpe':
            model = MAMNet_GSDPE(
                num_classes=2, pretrained=False, use_aux=True,
                reference_gsd=args.reference_gsd, use_contrast=True)
        ...
        for batch in tqdm(dataloader, desc="Inference"):
            ...
            if args.model_variant == 'hrda':
                if args.model_type == 'mamnet':
                    ...
                    outputs  = model.context_decoder(
                        features['feat_block11'], return_aux=False)
                else:
                ...
                if 'gsd' in batch:
                    ...
                    default_gsd = 0.3 if args.test_res == 'highres' else 0.6
                    gsd = torch.full(
                        (images.size(0),), default_gsd, device=device)
                outputs = model(images, gsd)

            else:
                # base, vanilla, fda, segdesic, iim, isw, mrfp_plus, fada
                outputs = model(images)
